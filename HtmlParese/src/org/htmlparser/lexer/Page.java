...
//
// $URL: http://svn.code.sf.net/p/htmlparser/code/trunk/lexer/src/main/java/org/htmlparser/lexer/Page.java $
...
/**
   ...
   * @exception ParserException If 
   * 1)the <code>connect()</code> method fails or
   * 2)an I/O error occurs opening the input stream or 
   * 3)the character set designated in the HTTP header is unsupported or or
   * 4)URL used to create the connection object supplied is not in the right
   * form or syntax 
   */
  ...
  ParserException
  {
    ...
    if(!isValidURL(connection)){
      throw new ParserException ("URL used is not valid: "
  ...
  /**
   * @param connection That would be used in set connection to update the 
   * value of mconnection field.
   * @return true if this connection is valid i.e. if the URL the connection
   * is based upon is valid in the sense that it has the right syntax and
   * format
   * @exception IllegalArgumentException if the argument supplied is null
   * This method checks for the validity of the URLConnection provided by 
   * checking the URL that is connection object is created upon has a 
   * valid format.
   * NOTE: Although URLConnection class itself takes care of many things by
   * throwing an MalformedURLException there are few cases which pass this test
   * but still are not valid URL.
   * SEE BUG: 
   * Creating a parser with an un-connectable HttpURLConnection - ID: 2899333 
   */
  private boolean isValidURL(URLConnection connection) {
    if (null == connection)
      throw new IllegalArgumentException ("connection cannot be null");
    boolean ret = true;
